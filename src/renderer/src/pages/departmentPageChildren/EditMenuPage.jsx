...
const EditMenuPage = () => {
  ...
  const handleSubmitData = () => {
    // mutate({ cartData, cartItems, cartId: cartData.id })
    dispatch(dialogActions.hadleOpenDialog('editCartConfirmation'))
  }
  ...
  return (
    <div className="flex flex-col  relative">
          ...
          <section className="w-[15rem] py-5 justify-end flex gap-3 flex-wrap max-h-[35rem] overflow-y-scroll">
            {data.items?.map((item) => (
              <button
                onClick={() => {
                  setError(null)
                  dispatch(cartActions.handleAddItem({ name: item.name, id: item.id }))
                }}
      ...
      <section className="w-full flex justify-center items-center gap-5 relative mt-5">
        ...
        <button
          onClick={() => {
            cartItems.length > 0 ? handleSubmitData() : setError('قم باختيار صنف على الأقل!')
          }}
